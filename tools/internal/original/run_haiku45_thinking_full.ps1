...
if ([string]::IsNullOrWhiteSpace($PreregContractPath)) {
  $PreregContractPath = Join-Path $Root "src\credit_recourse\configs\c4r_journal_extension_prereg_v5_haiku_thinking_full.json"
}
...
$ExpectedStatus = "LOCKED_FOURTH_AMENDMENT_AFTER_HAIKU45_THINKING_PILOT_BEFORE_FULL_CHARACTERIZATION"
...
if (-not $PlanOnly -and [string]::IsNullOrWhiteSpace($PreregEvidence)) {
  throw "-PreregEvidence is required before a live full characterization run."
}

$Scope = $Contract.full_scope
$Provider = $Contract.provider_contract
$Gates = $Contract.quality_gates
$DateTag = [DateTime]::Now.ToString("yyyyMMdd_HHmmss")
$RunLabel = "C4RHTF_C4C4RC6_L1_0p75_ICb_haiku45thinking_b2048_max4096_p50_n575_seed1_${DateTag}"
...
$AnalysisDir = Join-Path $Root "data\analysis\c4r_journal_extension\haiku_thinking_full_${DateTag}"
$BackendQc = Join-Path $AnalysisDir "haiku45_thinking_backend_contract.json"
$ArmQc = Join-Path $AnalysisDir "haiku45_thinking_full_arm_qc.json"
$Manifest = Join-Path $AnalysisDir "haiku45_thinking_full_manifest.json"
New-Item -ItemType Directory -Path $CheckpointDir,$AnalysisDir -Force | Out-Null
...
$Cmd = @(
  ...
  "--budget-contract-label", "C4RHTF_C4C4RC6_L1_0p75_ICb"
)

$RunManifest = [ordered]@{
  schema_version = "haiku45_thinking_full_runner_v1"
  ...
  arm_qc = $ArmQc
  confirmatory_inclusion_rule = "Eligible only when every unchanged full-run QC gate passes."
}
$RunManifest | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath $Manifest -Encoding UTF8

Write-Host "`n==== Haiku 4.5 thinking full characterization ====" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) {
  $RunManifest.status = "FAIL_RUNNER"
  $RunManifest.completed_utc = [DateTime]::UtcNow.ToString("o")
  $RunManifest | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath $Manifest -Encoding UTF8
  throw "Haiku thinking full runner failed."
}

Write-Host "`n==== Haiku 4.5 thinking full arm QC ====" -ForegroundColor Cyan
& $Py -m credit_recourse.verification.verify_c4r_journal_arm_contract `
  --archive-dir $ArchiveDir `
  --cohort-id "haiku45thinkingfull" `
  --budget-label "0p75" `
  --l1-budget "0.75" `
...
if (-not (Test-Path -LiteralPath $ArmQc -PathType Leaf)) {
  $RunManifest.status = "FAIL_QC_VERIFIER_TECHNICAL"
  $RunManifest.completed_utc = [DateTime]::UtcNow.ToString("o")
  $RunManifest | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath $Manifest -Encoding UTF8
  throw "Haiku thinking full QC verifier failed without producing an audit file."
}

$Qc = Get-Content -LiteralPath $ArmQc -Raw -Encoding UTF8 | ConvertFrom-Json
$RunManifest.completed_utc = [DateTime]::UtcNow.ToString("o")
$RunManifest.qc_summary = [ordered]@{
  verifier_status = [string]$Qc.status
...
if ($QcExitCode -eq 0 -and [string]$Qc.status -eq "PASS") {
  $RunManifest.status = "COMPLETE_PASS_FULL_QC"
  $RunManifest.confirmatory_inference_eligible = $true
  $RunManifest | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath $Manifest -Encoding UTF8
  Write-Host "COMPLETE_PASS_FULL_QC: $Manifest" -ForegroundColor Green
  exit 0
}

# A scientific QC failure is a completed result, not a software failure.
$RunManifest.status = "COMPLETE_FAIL_FULL_QC"
$RunManifest.confirmatory_inference_eligible = $false
$RunManifest | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath $Manifest -Encoding UTF8
Write-Warning "Haiku thinking full characterization completed but failed the unchanged preregistered QC gates."
Write-Host "COMPLETE_FAIL_FULL_QC: $Manifest" -ForegroundColor Yellow
Write-Host "The archive is preserved for feasibility reporting and exploratory downstream descriptions only." -ForegroundColor Yellow
exit 0
